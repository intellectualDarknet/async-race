...
import { SERVER_URL } from '../../consts-func-enum/constants';
import { Iwinner } from '../../epams/interface';
import { transferService } from '../../transferService/transfer-service';
import './signlerace.scss';

class SingleCar extends BaseComponent {
  public SingleCarInfo: Iwinner;

  public domCar: HTMLImageElement;
  ...
  public BtnA: HTMLButtonElement;

  public BtnB: HTMLButtonElement;

  public acceleration: number;

  public async ignite(id: number, status: string, value: boolean) {
    const response: Response = await fetch(SERVER_URL + `engine?id=${id}&status=${status}`, {
      method: 'PATCH',
    });
    const data = await response.json();
    this.transition = Math.round(500 / data.velocity);
    this.domCar.style.transition = `${this.transition}s transform linear`;
    this.acceleration = data.velocity;
    this.race(this.SingleCarInfo.id, 'drive', value);
  }

  public async stopEngine(id: number, status: string) {
    const response: Response = await fetch(SERVER_URL + `engine?id=${id}&status=${status}`, {
      method: 'PATCH',
    });
    const data = await response.json();
    console.log(data);
    this.domCar.style.transform = 'translate(calc(0px),23px)';
  }

  public async race(id: number, status: string, resultToShow: boolean) {
    const start = Date.now();
    this.domCar.style.transform = 'translate(calc(100vw - 290px),23px)';
    let data;
    try {
      const response: Response = await fetch(SERVER_URL + `engine?id=${id}&status=${status}`, {
        method: 'PATCH',
      ...
      this.domCar.style.transform =
        this.transition * 1000 >= Date.now() - start
          ? `translate(calc( (100vw - 290px) / ${this.transition} *  ${(Date.now() - start) / 1000}),23px)`
          : 'translate(calc( (100vw - 290px)),23px)';
    } finally {
      transferService.ImgPositionNamingArray[this.SingleCarInfo.id] = this.SingleCarInfo.name;
      transferService.ImgPositionInfoArray[this.SingleCarInfo.id] = this.domCar.style.transform;
      this.BtnA.classList.toggle('disabled');
      this.domCar.style.transition = 'none';
    }

    if (!transferService.resultsArray.length && data && resultToShow) {
      this.SingleCarInfo.time = ((Date.now() - start) / 1000).toFixed(1);
      M.toast({
        html: `${this.SingleCarInfo.name}, ${this.SingleCarInfo.time}`,
        completeCallback() {
          M.Toast.dismissAll();
        },
      });
      transferService.resultsArray.push(JSON.stringify(this.SingleCarInfo));

      if (transferService.carNamingArray.includes(this.SingleCarInfo.name)) {
        const getWinner: Response = await fetch(SERVER_URL + `winners/${this.SingleCarInfo.id}`, {
          method: 'GET',
        });
        const winnerInfo = await getWinner.json();

        const updateWinner: Response = await fetch(SERVER_URL + `winners/${this.SingleCarInfo.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            name: this.SingleCarInfo.name,
            id: this.SingleCarInfo.id,
            color: this.SingleCarInfo.color,
            wins: winnerInfo.wins + 1,
            time: +this.SingleCarInfo.time > +winnerInfo.time ? +winnerInfo.time : +this.SingleCarInfo.time,
          }),
        });
        const updateWinnerInfo = await updateWinner.json();
        console.log('updateWinnerInfo', updateWinnerInfo);
      } else {
        transferService.carNamingArray.push(this.SingleCarInfo.name);
        transferService.winnersAmount++;
        const response: Response = await fetch(SERVER_URL + 'winners', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  public async selfDelete() {
    if (transferService.carNamingArray.includes(this.SingleCarInfo.name)) {
      const deleteWinner: Response = await fetch(SERVER_URL + `winners/${this.SingleCarInfo.id}`, {
        method: 'DELETE',
      });
      const deleteWinnerInfo = await deleteWinner.json();
      console.log(deleteWinnerInfo);
      delete transferService.carNamingArray[transferService.carNamingArray.indexOf(this.SingleCarInfo.name)];
      transferService.winnersAmount--;
    }

    const response: Response = await fetch(SERVER_URL + 'garage/' + `${this.SingleCarInfo.id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const data = await response.json();
    console.log(data);
    transferService.defineWinnerPage();
    transferService.deleteObs.broadcast();
  }

  public defineImgPosition() {
    if (transferService.ImgPositionNamingArray[this.SingleCarInfo.id]) {
      this.domCar.style.transform = transferService.ImgPositionInfoArray[this.SingleCarInfo.id];
      this.BtnA.classList.remove('disabled');
      this.BtnB.classList.add('disabled');
    }
  }

  public init(): void {
    this.SingleCarInfo = JSON.parse(this.getAttribute('car-info'));
    this.template = `
    <div class = 'single'>
      <div class="single__row">
        <button class="single__btn btn orange darken-4 single__select">Select</button>
        <button class="single__btn btn orange darken-4 single__remove">Remove</button>
        <div class = "single__title">${this.SingleCarInfo.name}</div>
      </div>
      <div class="single__row">
        <button class="single__btn btn orange darken-4 single__B col">A</button>
        <button class="single__btn btn orange darken-4 single__A col disabled">B</button>
        <svg class = "single__svg single__svg${this.SingleCarInfo.id}" id = '${this.SingleCarInfo.id}' status = 'stopped' version="1.0" 
        xmlns="http://www.w3.org/2000/svg"
        width="130px" height="90px" viewBox="0 0 1280.000000 640.000000"
        preserveAspectRatio="xMidYMid meet">
       <g transform="translate(0.000000,640.000000) scale(0.100000,-0.100000)"
       fill="#000000" stroke="none">
       <path fill = '${this.SingleCarInfo.color}' d="M3565 5336 c-106 -30 -101 -26 -108 -111 -4 -42 -9 -80 -12 -85 -6
       ...
       61 239 98 16 10 -216 242 -234 235z"/>
        ...
        <i class = "material-icons single__right">flag</i>
      </div>
    </div>
		`;
  }
  ...
  public initHosts(): void {
    this.BtnSelect = this.querySelector('.single__select');
    this.BtnRemove = this.querySelector('.single__remove');
    this.BtnA = this.querySelector('.single__A');
    this.BtnB = this.querySelector('.single__B');
    this.domCar = this.querySelector(`.single__svg${this.SingleCarInfo.id}`);
    this.defineImgPosition();
  }

  public initHandlers(): void {
    this.subscriptions.push(
      new Subscription<MouseEvent>(this.BtnRemove, 'click', () => {
        transferService.defineRaceResetBtn.delete(this.SingleCarInfo.name);
        delete transferService.ImgPositionInfoArray[this.SingleCarInfo.id];
        delete transferService.ImgPositionNamingArray[this.SingleCarInfo.id];
        this.selfDelete();
      }),

      new Subscription<MouseEvent>(this.BtnSelect, 'click', () => {
        transferService.updateObs.broadcast(this.SingleCarInfo);
      }),

      new Subscription<MouseEvent>(this.BtnB, 'click', () => {
        this.BtnB.classList.toggle('disabled');
        this.ignite(this.SingleCarInfo.id, 'started', false);
        // transferService.ImgPositionInfoArray.push()
      }),

      new Subscription<MouseEvent>(this.BtnA, 'click', () => {
        this.BtnA.classList.toggle('disabled');
        this.BtnB.classList.toggle('disabled');
        this.stopEngine(this.SingleCarInfo.id, 'stopped');
        delete transferService.ImgPositionInfoArray[this.SingleCarInfo.id];
        delete transferService.ImgPositionNamingArray[this.SingleCarInfo.id];
      })
    );
    this.watchers.push(
      new Watcher(transferService.resetButtonsObs, () => {
        this.BtnA.classList.add('disabled');
        this.BtnB.classList.remove('disabled');
        this.stopEngine(this.SingleCarInfo.id, 'stopped');
      }),

      new Watcher(transferService.raceObs, () => {
        this.BtnB.classList.add('disabled');
        this.BtnB.classList.add('disabled');
        this.ignite(this.SingleCarInfo.id, 'started', true);
      }),

      new Watcher(transferService.raceResetObs, (str: string) => {
        switch (str) {
          case 'add':
            transferService.defineRaceResetBtn.add(this.SingleCarInfo.name);
            transferService.toRacesResetObs.broadcast('race');
            break;
          case 'remove':
            transferService.defineRaceResetBtn.delete(this.SingleCarInfo.name);
            transferService.toRacesResetObs.broadcast('reset');
            delete transferService.ImgPositionInfoArray[this.SingleCarInfo.id];
            delete transferService.ImgPositionNamingArray[this.SingleCarInfo.id];
            break;
          default:
...
export const singleCarComponent = new Component('app-singlecar', SingleCar);
