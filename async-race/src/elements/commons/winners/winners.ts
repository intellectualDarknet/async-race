...
class WinnerComponent extends BaseComponent {
  public winnerTitle: HTMLElement;

  public insertPlace: HTMLElement;

  public numberBtn: HTMLElement;

  public PagetoChange: HTMLElement;

  public carBtn: HTMLElement;

  public nameBtn: HTMLElement;

  public winsBtn: HTMLElement;

  public timeBtn: HTMLElement;

  public paginationLeft: HTMLButtonElement;

  public paginationRight: HTMLButtonElement;

  public nameFlag: 'asc' | 'desc' = 'asc';

  public winsFlag: 'asc' | 'desc' = 'asc';

  public timeFlag: 'asc' | 'desc' = 'asc';

  public colorFlag: 'asc' | 'desc' = 'asc';

  public sortAdress = '';

  public svg(a: string) {
    return `
    <svg class = "single__svg single__svg"  status = 'stopped' version="1.0" 
       xmlns="http://www.w3.org/2000/svg"
       width="120px" height="90px" viewBox="0 0 1280.000000 640.000000"
       preserveAspectRatio="xMidYMid meet">
      <g transform="translate(0.000000,640.000000) scale(0.100000,-0.100000)"
      fill="#000000" stroke="none">
      <path fill = '${a}' d="M3565 5336 c-106 -30 -101 -26 -108 -111 -4 -42 -9 -80 -12 -85 -6
  ...
  public async getWinners() {
    ...
    transferService.winnersAmount = data.length;
    this.winnerTitle.textContent = `Winners ${data.length}`;
    this.defineDisables();
  }

  public flagConvertion(a: string) {
    if ('desc' === a) {
      return 'asc';
    }
    return 'desc';
  ...
  public async SortBy(str: string, opt: string) {
    this.sortAdress = `${str}${opt}`;
    const neededResponse: Response = await fetch(SERVER_URL + `winners?_limit=10&_page=${transferService.winnersPage}${this.sortAdress}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    });
    const data = await neededResponse.json();
    this.insertPlace.innerHTML = data
      .map(
        (elem: Iwinner, index: number) => `
    <tr>
      <th class="indigo lighten-3 winner__number">${index + 1 + (transferService.winnersPage - 1) * 10}</th>
      <th class="indigo lighten-3 winner__car">${this.svg(elem.color)}</th>
      <th class="indigo lighten-3 winner__name">${elem.name}</th>
      <th class="indigo lighten-3 winner__wins">${elem.wins}</th>
      <th class="indigo lighten-3 winner__time">${elem.time}</th>
    </tr>
    `
      )
      .join('');
  ...
  public defineDisables() {
    if (transferService.winnersPage >= Math.ceil(transferService.winnersAmount / 10)) {
      this.paginationRight.classList.add('disabled');
    } else {
      this.paginationRight.classList.remove('disabled');
    }
    if (transferService.winnersPage == 1) {
  ...
  public async gatherResults() {
    const response: Response = await fetch(SERVER_URL + `winners?_limit=10&_page=${transferService.winnersPage}${this.sortAdress}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  public init(): void {
    ...
    <div class="winner">
      <div class="winner__wrapper">
        ...
        <h3 class="winner__title">Winners ${transferService.winnersAmount}</h3>
        <h4 class="winner__page">Page ${transferService.winnersPage}</h4>
        <table class="winner__tablet">
          <thead class="winner__thead">
            <tr class="winner__trow">
              <th class="indigo lighten-3 winner__number">Number</th>
              <th class="indigo lighten-3 winner__car">Car</th>
              <th class="indigo lighten-3 winner__name">Name</th>
              <th class="indigo lighten-3 winner__wins">Wins</th>
              <th class="indigo lighten-3 winner__time">Best time(s)</th>
            </tr>
          </thead>
          <tbody class = winner__insertPlace>
          </tbody>
        </table>

        <div class ="winner__pagination">
          <div class = "winner__wrapper-pagination">
            <button class = "btn pagination__left">prev</button>
            <button class = "btn pagination__right">next</button>
          </div>
        </div>
  ...
  public initHosts(): void {
    this.insertPlace = this.querySelector('.winner__insertPlace');
    this.numberBtn = this.querySelector('.winner__number');
    this.carBtn = this.querySelector('.winner__car');
    this.nameBtn = this.querySelector('.winner__name');
    this.winsBtn = this.querySelector('.winner__wins');
    this.timeBtn = this.querySelector('.winner__time');
    this.paginationLeft = this.querySelector('.pagination__left');
    this.paginationRight = this.querySelector('.pagination__right');
    this.PagetoChange = this.querySelector('.winner__page');
    this.winnerTitle = this.querySelector('.winner__title');
  }
  ...
  public initHandlers(): void {
    ...
    this.subscriptions.push(
      new Subscription<MouseEvent>(this.carBtn, 'click', () => {
        this.colorFlag = this.flagConvertion(this.colorFlag);
        this.SortBy('&_sort=color&_order=', this.colorFlag);
      }),
      new Subscription<MouseEvent>(this.nameBtn, 'click', () => {
        this.nameFlag = this.flagConvertion(this.nameFlag);
        this.SortBy('&_sort=name&_order=', this.nameFlag);
      }),
      new Subscription<MouseEvent>(this.winsBtn, 'click', () => {
        this.winsFlag = this.flagConvertion(this.winsFlag);
        this.SortBy('&_sort=wins&_order=', this.winsFlag);
      }),
      new Subscription<MouseEvent>(this.timeBtn, 'click', () => {
        this.timeFlag = this.flagConvertion(this.timeFlag);
        this.SortBy('&_sort=time&_order=', this.timeFlag);
      }),

      new Subscription<MouseEvent>(this.paginationLeft, 'click', () => {
        transferService.winnersPage--;
        this.gatherResults();
        this.defineDisables();
        this.PagetoChange.textContent = `Page ${transferService.winnersPage}`;
      }),
...
export const winnerComponent = new Component('app-winner', WinnerComponent);
